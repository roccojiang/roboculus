...
public class ObjectManipulator : MonoBehaviour {
  ...
  void GrabHoverObject(GameObject grbObj, Vector3 controllerPos,
                       Quaternion controllerRot) {
    localGrabOffset = Quaternion.Inverse(controllerRot) *
                      (grabObject.transform.position - controllerPos);
    localGrabRotation =
  ...
  void ReleaseObject() {
    if (grabObject.GetComponent<GrabObject>()) {
      if (grabObject.GetComponent<GrabObject>().objectManipulationType ==
  ...
  IEnumerator StartDemo() {
    ...
    while (timer <= fadeTime) {
      ...
      if (passthrough) {
        passthrough.colorMapEditorBrightness =
  ...
  void FindHoverObject(Vector3 controllerPos, Quaternion controllerRot) {
    ...
    if (objectInfo && objectInstructionsLabel) {
      ...
      if (hoverObject && !grabObject) {
        ...
        objectInfo.position =
            Vector3.Lerp(targetPos, objectInfo.position, grabTime);
        ...
        objectInfo.position =
            Vector3.Lerp(objectInfo.position, targetPos, grabTime);
        ;
        ...
        if (grabObject.GetComponent<GrabObject>())
          showLaser =
              grabObject.GetComponent<GrabObject>().showLaserWhileGrabbed;
      }
    }
  ...
  void ManipulateObject(GameObject obj, Vector3 controllerPos,
                        Quaternion controllerRot) {
    ...
    if (obj.GetComponent<GrabObject>()) {
      ...
      case GrabObject.ManipulationType.ForcedHand:
        obj.transform.position =
            Vector3.Lerp(obj.transform.position, controllerPos, grabTime);
        obj.transform.rotation =
            Quaternion.Lerp(obj.transform.rotation, controllerRot, grabTime);
        break;
      ...
      case GrabObject.ManipulationType.HorizontalScaled:
        ...
        obj.transform.rotation = Quaternion.Euler(0, rotationOffset, 0) *
      ...
      case GrabObject.ManipulationType.VerticalScaled:
        ...
        obj.transform.rotation = Quaternion.LookRotation(
            Vector3.up, Quaternion.Euler(0, rotationOffset, 0) * newUp);
        break;
      case GrabObject.ManipulationType.Menu:
        Vector3 targetPos =
            handGrabPosition + (handGrabRotation * Vector3.forward * 0.4f);
        Quaternion targetRotation =
            Quaternion.LookRotation(targetPos - camGrabPosition);
        obj.transform.position =
            Vector3.Lerp(obj.transform.position, targetPos, grabTime);
        obj.transform.rotation =
            Quaternion.Lerp(obj.transform.rotation, targetRotation, grabTime);
        break;
      default:
        useDefaultManipulation = true;
        break;
  ...
  void ClampGrabOffset(ref Vector3 localOffset, float thumbY) {
    Vector3 projectedGrabOffset =
        localOffset + Vector3.forward * thumbY * 0.01f;
